...
interface TemplateBlockExerciseApi {
  id: string;
  block_id: string;
  exercise_id: string;
...
interface TemplateBlockApi {
  id: string;
  template_id: string;
  block_type: string;
  order: number;
  rest_after_seconds: number | null;
  exercises: TemplateBlockExerciseApi[];
}
...
interface TemplateListApi {
  items: Omit<WorkoutTemplateApi, "blocks">[];
  total: number;
  page: number;
  page_size: number;
}
...
function transformTbe(raw: TemplateBlockExerciseApi): TemplateBlockExercise {
  return {
    id: raw.id,
...
function transformBlock(raw: TemplateBlockApi): TemplateBlock {
  return {
    id: raw.id,
...
function transformTemplate(raw: WorkoutTemplateApi): WorkoutTemplate {
  return {
    id: raw.id,
...
function transformTemplateSummary(
  raw: Omit<WorkoutTemplateApi, "blocks">,
): WorkoutTemplateSummary {
  return {
    id: raw.id,
    userId: raw.user_id ?? undefined,
...
export async function fetchTemplates(q?: string, page = 1): Promise<PaginatedTemplates> {
...
export async function fetchTemplate(id: string): Promise<WorkoutTemplate> {
  const raw = await apiFetch<WorkoutTemplateApi>(`/templates/${id}`);
...
export async function createTemplate(data: {
  ...
  const raw = await apiFetch<WorkoutTemplateApi>("/templates", {
    method: "POST",
    body: JSON.stringify({
      name: data.name,
...
export async function addBlock(
  ...
  const raw = await apiFetch<TemplateBlockApi>(`/templates/${templateId}/blocks`, {
    method: "POST",
    body: JSON.stringify({
...
export async function updateBlock(
  ...
  const raw = await apiFetch<TemplateBlockApi>(
...
export async function deleteBlock(templateId: string, blockId: string): Promise<void> {
...
export async function reorderBlocks(
  ...
  const raw = await apiFetch<WorkoutTemplateApi>(
    `/templates/${templateId}/blocks/reorder`,
...
export async function addExerciseToBlock(
  ...
  const raw = await apiFetch<TemplateBlockExerciseApi>(
...
// Re-export types so screens can import from a single place
export type {
  WorkoutTemplate,
  WorkoutTemplateSummary,
  TemplateBlock,
...
type _CompletedSet = import("@colossus/types").CompletedSet;
type _CompletedWorkout = import("@colossus/types").CompletedWorkout;

function transformCompletedSet(raw: CompletedSetApi): _CompletedSet {
  return {
    id: raw.id,
...
function transformCompletedWorkout(raw: CompletedWorkoutApi): _CompletedWorkout {
  return {
    id: raw.id,
    userId: raw.user_id ?? undefined,
...
export async function startWorkout(templateId?: string): Promise<_CompletedWorkout> {
...
export async function fetchWorkout(id: string): Promise<_CompletedWorkout> {
  const raw = await apiFetch<CompletedWorkoutApi>(`/workouts/${id}`);
  return transformCompletedWorkout(raw);
}

export async function finishWorkout(id: string): Promise<_CompletedWorkout> {
...
export async function updateSet(
  workoutId: string,
  setId: string,
  params: Partial<LogSetParams>,
): Promise<_CompletedSet> {
  const body: Record<string, unknown> = {};
  if ("repsCompleted" in params) body.reps_completed = params.repsCompleted ?? null;
  if ("weight" in params) body.weight = params.weight ?? null;
  if ("isWarmup" in params) body.is_warmup = params.isWarmup;
  if ("isFailure" in params) body.is_failure = params.isFailure;
  if ("rpe" in params) body.rpe = params.rpe ?? null;
  if ("notes" in params) body.notes = params.notes ?? null;
  const raw = await apiFetch<CompletedSetApi>(
